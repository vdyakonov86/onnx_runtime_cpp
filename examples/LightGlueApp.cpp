...
int main(int argc, char* argv[])
{
    if (argc != 5) {
        std::cerr
            << "Usage: [apps] [path/to/onnx/super/point] [path/to/onnx/light/glue] [path/to/image1] [path/to/image2]"
            << std::endl;
        return EXIT_FAILURE;
    }

    const std::string ONNX_MODEL_PATH = argv[1];
    const std::string LIGHTGLUE_ONNX_MODEL_PATH = argv[2];
    ...
    Ort::LightGlue lightGlueOsh(LIGHTGLUE_ONNX_MODEL_PATH, 0);
    auto matches = lightGlueOsh.inference(lightGlueOsh, superPointResults[0], superPointResults[1], images[0].size());
